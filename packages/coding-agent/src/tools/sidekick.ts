...
const sidekickSchema = type({
	...
	"block?": type("boolean").describe(
		"Whether this handoff blocks until the sidekick finishes and returns its\nreport directly (default: true). If the wait times out or is interrupted\nfirst, the handoff continues in the background and its report arrives in a\n<subagent_completion_notification>; do not assume it finished before that\nnotification.",
	),
});
